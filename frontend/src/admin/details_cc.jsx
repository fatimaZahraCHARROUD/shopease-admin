 ...
 const navigate = useNavigate(); 
  ...
  useEffect(() => {
    const fetchProduits = async () => {
      try {
        const response = await axios.get(`http://localhost:8800/produit_cc/${id}`);
    ...
    fetchProduits();
  }, [id]); // Recharger les produits quand id_cc change

  return (
    <div style={{ marginLeft:"180px"}}>
      <h1>Détails des produits pour la commande {id}</h1> {/* Utilisez id_cc ici */}
